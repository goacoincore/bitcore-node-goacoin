...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/bitcore-node-goacoin.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update bitcore-node-goacoin.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
            'bitcore-lib-goacoin': 'goacoincore/bitcore-lib-goacoin',
			'bitcore-node-goacoin': 'goacoincore/bitcore-node-goacoin'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/bitcore-node-goacoin.json');
