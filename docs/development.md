...
To develop bitcore-node-goacoin:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-goacoin.git
...
We now will setup symlinks in `bitcore-node-goacoin` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-goacoin
ln -s ~/bitcoind-rpc-goacoin
```
...
cd devnode
mkdir node_modules
touch bitcore-node-goacoin.json
touch package.json
```

Edit `bitcore-node-goacoin.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-goacoin/bin/bitcore-node-goacoin start
```